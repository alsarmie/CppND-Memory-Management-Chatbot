...
/**
 * The ChatBot Rule of Five implementation is based on this awesome SO answer:
 * https://stackoverflow.com/questions/3279543/what-is-the-copy-and-swap-idiom/3279550#3279550
 *  It improves over the naive/basic implementation.
 * Copy-and-swap idiom is used here.
 */
//!
/** !DISCLOSURE: The Copy Assignment operator is commented as it creates an
 * ambiguity when calling '='. (Expected per the type of implementation)
 * ChatBot &ChatBot::operator=(ChatBot chatbot) and
 * ChatBot &ChatBot::operator=(ChatBot &&chatbot) noexcept
 * Seem to rank the same. As well, the following was avoided due to optimization
 *reasons:
 *
 * ChatBot &ChatBot::operator=(const ChatBot &chatbot){
 *    ChatBot temp(chatbot);
 *    swap(*this,temp);
 *    return *this;
 * }
 *
 *ChatBot &ChatBot::operator=(ChatBot chatbot) could work for both cases (move
 *and copy).
 *
 */
//!
 ...
 *  b) DRY
 * We also avoid/solve C.62 (cpp core guidelines):
 *
 *! Copy-and-swap idiom is used here
 * @param chatbot
 * @return ChatBot&
...
void ChatBot::ReceiveMessageFromUser(std::string &message) {
  ...
  // First we check if the message is a valid message. At least the message has
  // to be equal or larger than the shortest keyword.
  if (message.size() >= GetMinKeywordSize())
    for (size_t i = 0; i < _currentNode->GetNumberOfChildEdges(); ++i) {
      GraphEdge *edge = _currentNode->GetChildEdgeAtIndex(i);
      for (auto keyword : edge->GetKeywords()) {
        std::transform(message.begin(), message.end(), s1.begin(), ::tolower);
        (message.find(keyword) != std::string::npos) ? token++ : 0;
