          ...
          if (type->second == "EDGE") {
            ...
            if (parentToken != tokens.end() && childToken != tokens.end()) {
              ...
              auto parentNode = std::find_if(
                  ...
                  [&parentToken](unique_ptr<GraphNode> const &node) {
                    return node->GetID() == std::stoi(parentToken->second);
                  });
              auto childNode = std::find_if(
                  _nodes.begin(), _nodes.end(),
                  [&childToken](unique_ptr<GraphNode> const &node) {
                    return node->GetID() == std::stoi(childToken->second);
                  });

              // Create new edge
              // Dereference iterator to gain access to the
              // unique_ptr. We use get() to access the raw
              // pointer as no ownership transfer is required.
              unique_ptr<GraphEdge> edge = make_unique<GraphEdge>(id);
              edge->SetChildNode((*childNode).get()); // uses handle, no ownership transfer.
              edge->SetParentNode((*parentNode).get());// uses handle, no ownership transfer.
