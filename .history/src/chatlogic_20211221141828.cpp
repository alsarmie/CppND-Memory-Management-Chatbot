        ...
        if (idToken != tokens.end()) {
          ...
          if (type->second == "NODE") {
            ...
            auto newNode = std::find_if(
                ...
                [&id](unique_ptr<GraphNode> const &node) { return node->GetID() == id; });
          ...
          if (type->second == "EDGE") {
            ...
            if (parentToken != tokens.end() && childToken != tokens.end()) {
              ...
              auto childNode = std::find_if(
                  _nodes.begin(), _nodes.end(), [&childToken](unique_ptr<GraphNode> const &node) {
                    return node->GetID() == std::stoi(childToken->second);
                  });

              // create new edge
              GraphEdge *edge = new GraphEdge(id);
              edge->SetChildNode((*childNode).get()); //dereference iterator to gain access to the unique_ptr. We use get() to access the raw pointer as no wonership transfer is required.
